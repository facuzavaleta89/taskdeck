      ...
      <Modal onClose={onClose} size="md">
        <div className="p-6 space-y-5">
          ...
          <div>
            <input
              ...
              className="w-full text-lg font-bold text-slate-800 bg-transparent focus:outline-none border-b-2 border-transparent focus:border-blue-400 pb-1 placeholder-slate-300 transition-colors"
          ...
          <Field label="Descripción">
            <textarea
              ...
              rows={3}
              className="w-full text-sm text-slate-700 bg-slate-50 placeholder-slate-400 border border-slate-200 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none transition-colors"
            />
          </Field>
          ...
          <Field label="Fecha de vencimiento">
            <input
              ...
              className="text-sm text-slate-700 bg-white border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
            />
            {dueDate && (
              <button
                onClick={() => setDueDate('')}
          ...
          >
            ...
            <div className="space-y-1">
              {checklistItems.map(item => (
                <div key={item.id} className="flex items-center gap-3 group py-1 px-1 rounded-lg hover:bg-slate-50 transition-colors">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className={cn('text-sm flex-1', item.completed ? 'line-through text-slate-400' : 'text-slate-700')}>
                    {item.text}
                  </span>
                  <button
                    onClick={() => handleDeleteItem(item.id)}
                    className="w-6 h-6 flex items-center justify-center rounded text-slate-300 hover:text-red-400 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0"
                  >
            ...
            {addingItem && (
              <div className="mt-2 animate-scale-in">
                <input
                  ...
                  autoFocus
                  className="w-full text-sm text-slate-800 bg-white placeholder-slate-400 border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 mb-2"
                />
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-slate-700 text-xs px-2 transition-colors"
                  >
                    Cancelar
                  </button>
                </div>
        ...
        <div className="px-6 pb-6 flex items-center justify-between border-t border-slate-100 pt-4">
          <button
            ...
            className="text-slate-400 hover:text-red-500 text-sm transition-colors flex items-center gap-1.5"
          >
